...
- **Panels fold away.** Every panel on the Character and Edit pages collapses to its header,
  each level inside the feats list folds on its own, and the talent picker's trees fold
  individually. Everything starts open and your choices are remembered across tabs and
  reloads — except that a panel or level owing you something is held open until you deal with
  it, then returns to however you left it. A folded level still says how many choices are
  waiting inside. Picker trees always reopen expanded, since you want to see what's on offer.
